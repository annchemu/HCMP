...
class Rtk_Management extends Home_controller {
    ...
    public function national_rtk_allocation() {
        ...
        $data['allocations'] = $this->_allocation();
...
function fill_percentage_tables(){
    $month = date('mY', time());  
    $sql = "insert into rtk_county_percentage (county_id,facilities,reported,month) values($id,$facilities,'0','$month')";
...
public function select_where(){
    $q = "select district_id from rtk_district_percentage where ((reported/facilities)*100)>50";
    echo "$q";die();
}
...
//Function to get all the facilities and the districts
    ...
    public function get_county_percentages(){
        ...
        foreach ($result as $value) {
            $id=$value['id'];
            $perc = $this->rtk_summary_county($id,$year,$month);
            array_push($percentage, $perc);
        }

        $reported = array();
        foreach ($percentage as $value) {
            $val = $value['reported_percentage'];
            array_push($reported, $val);
        }
        echo "<pre>";
...
public function get_report($facility_code) {
         ...
         */ {
          ...
          $data['content_view'] = "rtk/rtk/dpp/fcdrr";
...
//           echo "<pre>";print_r($lab_count);echo "</pre>";
            ...
            public function get_counties_facilities_details($countyid){
                ...
                facility_amc
                where
                lab_commodity_details.district_id = districts.id
                ...
                and lab_commodity_orders.order_date between '2014-08-01' and '2014-08-31'
                and facilities.rtk_enabled = 1
                and districts.id = facilities.district
                and facilities.facility_code = lab_commodity_orders.facility_code
                and facility_amc.facility_code = facilities.facility_code";
                $res = $this->db->query($sql);
        ...
        //             and districts.county = counties.id
        //             and counties.id = 27
        //             and lab_commodity_details.facility_code = $fcode
        //             and lab_commodity_details.order_id = lab_commodity_orders.id
        //             and lab_commodity_orders.order_date between '2014-08-01' and '2014-08-31'
        //             and facilities.rtk_enabled = 1
        //             and districts.id = facilities.district
        ...
        // }        
                ...
                $data['title'] = 'Meru County List Quantity Requested';
                $data['banner_text'] = 'Quantities Requested in Meru County';
...
";
    ...
    public function edit_lab_order_details($order_id, $msg = NULL) {
        ...
        $data['content_view'] = "rtk/rtk/dpp/fcdrr_edit";
    ...
    //generate pdf
        ...
        // $data['content_view'] = "rtk/lab_order_details_v";
                    $data['order_id'] = $order_id;
                    $data['content_view'] = "rtk/rtk/dpp/fcdrr_report";
                    $data['banner_text'] = "Lab Commodity Order Details";
...
//echo '<pre>';print_r($result);die;
        ...
        public function rtk_send_message() {
            ...
            //Sub C with more than 75% reporting
                $sql = "select distinct district_id from  rtk_district_percentage  where  month = '$month' and ((reported / facilities) * 100) > 75";
            ...
            //Sub C with less than 50% reporting   
                $sql = "select distinct district_id from  rtk_district_percentage  where  month = '$month' and ((reported / facilities) * 100) > 50";
                $districts = $this->db->query($sql)->result_array();
                $emails = array();
                foreach ($districts as $value) {
                    $dist = $value['district_id'];
            ...
            //Sub C with less than 25% reporting   
                $sql = "select distinct district_id from  rtk_district_percentage  where  month = '$month' and ((reported / facilities) * 100) > 25";
                $districts = $this->db->query($sql)->result_array();
                $emails = array();
                foreach ($districts as $value) {
                    $dist = $value['district_id'];
            ...
            //C with more than 75% reporting   
                $sql = "select distinct county_id from  rtk_county_percentage  where  month = '$month' and ((reported / facilities) * 100) >75";
            ...
            //C with less than 50% reporting 
                $sql = "select distinct county_id from  rtk_county_percentage  where  month = '$month' and ((reported / facilities) * 100) <50";
                $counties = $this->db->query($sql)->result_array();
                $emails = array();
                foreach ($counties as $value) {
                    $county = $value['county_id'];
            ...
            //C with 0% reporting 
            $sql = "select distinct county_id from  rtk_county_percentage  where  month = '$month' and ((reported / facilities) * 100) = '0";
            $districts = $this->db->query($sql)->result_array();
            $emails = array();
            foreach ($districts as $value) {
                $county = $value['county_id'];
            ...
            $sql = "select distinct district_id from  rtk_district_percentage  where  month = '$month' and ((reported / facilities) * 100) = 0";
            $districts = $this->db->query($sql)->result_array();
            $emails = array();
            foreach ($districts as $value) {
                $dist = $value['district_id'];
    ...
    public function get_county_percentage(){
        // /$
    }
...
//        echo"<pre>";print_r($res->result_array());die;
    ...
    function _national_reports_sum($year, $month) {
        ...
        AND districts.county = counties.id
        AND lab_commodity_orders.order_date BETWEEN '$firstdate' AND '$lastdate'
        AND lab_commodities.id in (select lab_commodities.id from lab_commodities,lab_commodity_categories where lab_commodities.category = lab_commodity_categories.id and lab_commodity_categories.active = '1')
        Group By counties.county,lab_commodities.commodity_name";    
